...
from super_agents.performance import initialize_performance_optimizations, optimize_for_command
initialize_performance_optimizations()

# Use lazy imports for better startup performance
from super_agents.performance.lazy_loader import lazy_import
from super_agents.performance.caching import cached_file_read
...
# Create console and Panel lazily
def get_console():
...
@click.option('--save-baseline', is_flag=True, help="Save performance baseline for future comparisons")
@click.version_option(version=__version__)
@click.pass_context
def main(ctx, stop, status, list, profile, save_baseline):
    """
    ...
    # Store profiling options in context for subcommands
    ctx.ensure_object(dict)
    ctx.obj['profile'] = profile
    ctx.obj['save_baseline'] = save_baseline
    
    # If no command specified, run default behavior
    if ctx.invoked_subcommand is None:
        # Default behavior: init if needed, start KM
        if profile:
            from super_agents.utils.profiler import profile_command
            with profile_command("main", save_baseline=save_baseline):
                run_default()
        else:
            run_default()


@optimize_for_command("main")
def run_default():
...
def init(ctx, force):
    ...
    profile = ctx.obj.get('profile', False) if ctx.obj else False
    save_baseline = ctx.obj.get('save_baseline', False) if ctx.obj else False
    
    if profile:
        from super_agents.utils.profiler import profile_command
        with profile_command("init", save_baseline=save_baseline):
            success = initialize_project(force=force)
    else:
        success = initialize_project(force=force)
...
@click.pass_context
@optimize_for_command("upgrade")
def upgrade(ctx, backup_dir):
    ...
    if profile:
        from super_agents.utils.profiler import profile_command
        with profile_command("upgrade", save_baseline=save_baseline):
            success = upgrade_project(backup_dir=backup_dir)
    else:
        success = upgrade_project(backup_dir=backup_dir)
